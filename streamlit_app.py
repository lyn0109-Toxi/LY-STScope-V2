...
st.set_page_config(page_title="LY-STScope Ver.2 REIT", layout="wide")
...
SAMPLE_REITS: list[ReitRecord] = [
    ReitRecord("PLD", "Prologis", "Industrial", 111.40, 3.55, 19.8, 72.0, -3.2, 5.1, 97.2, 1.02, -0.62),
    ReitRecord("O", "Realty Income", "Retail Net Lease", 56.20, 5.65, 13.4, 76.0, -8.5, 5.5, 98.6, 0.82, -0.78),
    ReitRecord("EQIX", "Equinix", "Data Center", 792.50, 2.15, 24.6, 64.0, 5.4, 4.2, 96.1, 0.94, -0.48),
    ReitRecord("AVB", "AvalonBay Communities", "Residential", 197.80, 3.40, 17.1, 68.0, -2.1, 4.8, 95.8, 0.88, -0.56),
    ReitRecord("WELL", "Welltower", "Healthcare", 126.70, 2.10, 27.2, 61.0, 9.0, 5.8, 87.5, 1.04, -0.44),
    ReitRecord("PSA", "Public Storage", "Storage", 288.30, 4.15, 18.7, 71.0, 1.8, 3.9, 92.8, 0.72, -0.40),
    ReitRecord("SPG", "Simon Property Group", "Retail Mall", 159.60, 5.00, 12.8, 67.0, -6.0, 5.9, 95.5, 1.35, -0.69),
    ReitRecord("BXP", "BXP", "Office", 66.40, 5.90, 9.2, 82.0, -18.0, 7.1, 88.6, 1.28, -0.84),
]


def reit_frame() -> pd.DataFrame:
    return pd.DataFrame([record.__dict__ for record in SAMPLE_REITS])
...
def valuation_label(row: pd.Series) -> str:
    score = 0
    if row["nav_discount"] <= -8:
        score += 1
    if row["price_to_ffo"] <= 14:
        score += 1
    if row["affo_payout"] <= 75:
        score += 1
    if row["debt_to_ebitda"] >= 6.5:
        score -= 1
    if score >= 2:
...
def valuation_tab(df: pd.DataFrame) -> None:
    ...
    radar = pd.DataFrame(
        [
            {"Dimension": "Income", "Score": min(10, row["dividend_yield"] * 1.5)},
            {"Dimension": "Valuation", "Score": max(0, min(10, 6 - row["nav_discount"] / 3))},
            {"Dimension": "Balance Sheet", "Score": max(0, min(10, 10 - row["debt_to_ebitda"]))},
            {"Dimension": "Operating Quality", "Score": max(0, min(10, row["occupancy"] / 10))},
        ]
    )
    chart = (
        alt.Chart(radar)
...
def portfolio_tab(df: pd.DataFrame) -> None:
    ...
    display = selected_df[["symbol", "company", "sector", "Weight", "dividend_yield", "price_to_ffo", "nav_discount", "beta"]].copy()
...
def main() -> None:
    ...
    hero()
    df = reit_frame()
    tabs = st.tabs(["Overview", "Valuation", "Macro Sensitivity", "Portfolio", "Guide"])
    with tabs[0]:
        overview_tab(df)
    with tabs[1]:
        valuation_tab(df)
